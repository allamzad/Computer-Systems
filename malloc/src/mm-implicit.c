...
void *split_block(void *block, size_t size, size_t block_size) {
    set_header((block_t *) block, size, true);
...
void coalesce_blocks() {
    ...
    while (mm_heap_last != NULL && curr <= mm_heap_last) {
        if (!is_allocated(prev) && !is_allocated(curr)) {
            size_t new_block_size = get_size(prev) + get_size(curr);
            if (curr == mm_heap_last) {
                mm_heap_last = prev;
            }

            set_header(prev, new_block_size, false);
            curr = (void *) prev + new_block_size;
        }
        else {
...
void *mm_malloc(size_t size) {
    ...
    if (block != NULL) {
        ...
        if (size < block_size) {
            return split_block(block, size, block_size);
        }
        else {
...
void *mm_realloc(void *old_ptr, size_t size) {
    ...
    size_t old_size = get_size(block_from_payload(old_ptr)) - sizeof(block_t);
    void *new_ptr;
    if (old_size < size) {
