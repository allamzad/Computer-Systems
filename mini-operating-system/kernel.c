...
void process_setup(pid_t pid, const char* program_name) {
    ...
    for (auto seg = pgm.begin(); seg != pgm.end(); ++seg) {
        for (uintptr_t a = round_down(seg.va(), PAGESIZE);
            ...
            memory_map(ptable[pid].pagetable, a, p, PTE_PWU);
        }
    }

    // initialize data in loadable segments
    for (auto seg = pgm.begin(); seg != pgm.end(); ++seg) {
        memset(memory_virtual_to_physical(ptable[pid].pagetable, (uintptr_t) seg.va()), 0, seg.size());
        memcpy(memory_virtual_to_physical(ptable[pid].pagetable, (uintptr_t) seg.va()), seg.data(), seg.data_size());
    }
    ...
    uintptr_t p = (uintptr_t)kalloc(PAGESIZE);
    memory_map(ptable[pid].pagetable, stack_addr, p, PTE_PWU);
...
int syscall_page_alloc(uintptr_t addr) {
    ...
    memory_map(current->pagetable, addr, p, PTE_PWU);
